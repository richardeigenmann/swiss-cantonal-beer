...
export class CantonService {
  ...
  private readonly _cantons = signal<Canton[]>([
  ...
  get cantons(): Signal<Canton[]> {
    return this._cantons;
  }

  // A computed signal that generates a Map for fast lookups
  readonly cantonsMap: Signal<Map<string, Canton>> = computed(() => {
    const cantonMap = new Map<string, Canton>();
  ...
  getCantonByCode(code: string): Canton | undefined {
